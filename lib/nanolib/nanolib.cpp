...
void GPIO_Set(String tag_id, String pin){
  /*
   * Used to set GPIO pins high and low as an example using LEDs
   * Input: tag_id to request, pin to change (0-4)
   * When successful you should see a *AIR:tag_id,cmd_type(5a),cmd_resp(57),length(09),command requested(010401000503e803e8)
   */
  String message = "sdat 0 ";
  message += tag_id;
  message += " 12 "; //length of message
  message += "081255540255000A5A0";  //0812555402 means air command 55 is the set opcode 00 is reserved 0A is length-1 5A is command type
  message += pin;
  message += "0401000503E803E8"; //04 flash mode, 01 start high, 00 push pull, 05 repeat 5 times, 03e8 high for 1000ms, 03e8 low for 1000ms 
  //message += " 5000";
